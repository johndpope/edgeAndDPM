function multi_objectDt(DtType)
 ...
 %    cls = model.class;
	...
	% end
	% save mydata1 bilis;
%sheep_trainval bicycle_trainval1
	load '../VOCdevkit/VOC2011/ImageSets/Main/sheep_trainval1.txt';
	bicycle_trainval1 = sheep_trainval1;
	...
	for i = 1:size(bicycle_trainval1,1)
		if bicycle_trainval1(i,3) == 1
			...
			% im = imread(imageName);
			[bboxes , n] = readObjectBoxesFromXML(annotation,'sheep');%bicycle');
			fprintf('11NO. %d \n', i);
			%nothing = InteractiveDector(im,model,cls,200);
			if n>1
				im = imread(imageName);
				myshowboxes(bboxes ,'r' , im);
				pause;
				% myshowboxes(bboxes,[1,0,1],im);
				% text(0,-10,s);
	...
	if DtType == 1
		save personArea bilis;
		drawbilis(bilis,5);
	elseif DtType == 2
		save personPolars1 polars;
		drawbilis(polars,6);
	elseif DtType == 4
		save personArea bilis;
		drawbilis(bilis,5);
